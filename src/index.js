...
function createCard(nametext,emailtext,phonetext){
    ...
    card.className="rounded-xl h-max shadow-lg border-2 bg-white relative w-full xs:w-full";
    card.id=phonetext;
    ...
    image.className="bg-slate-50 pt-5 pb-3 px-5 rounded-l-xl xs:px-3";
    ...
    icon_div.className="rounded-full h-12 w-12 bg-blue-400 flex justify-center items-center";
    img_arr.appendChild(icon_div);
    let span=document.createElement("span");
    span.className="text-white font-nunito";
    icon_div.appendChild(span);
    let span_text=document.createTextNode(nametext.split(' ').map(x => x[0]).join(''));
    span.appendChild(span_text);

    //buttons
    let buttons_div=document.createElement("div");
    img_arr.appendChild(buttons_div);
    buttons_div.className="flex gap-2 mt-1";
    //phone button
    let phone_btn=document.createElement("button");
    phone_btn.className="rounded-full h-5 w-5 bg-white flex justify-center items-center border-[1px] border-primary";
    buttons_div.appendChild(phone_btn);
    let phone_span=document.createElement("span");
    phone_span.className="fa fa-phone text-primary text-xs ";
    phone_btn.appendChild(phone_span);
    //email button
    let email_btn=document.createElement("button");
    email_btn.className="rounded-full h-5 w-5 bg-white flex justify-center items-center border-[1px] border-blue-400";
    buttons_div.appendChild(email_btn);
    let email_span=document.createElement("span");
    email_span.className="fa fa-at text-blue-400 text-xs";
    email_btn.appendChild(email_span);

    // details
    let details_div=document.createElement("div");
    details_div.className="pt-6 pb-3 px-2 xs:w-full";
    container.appendChild(details_div);

    //buttons-arr
    let buttons_arr=document.createElement("div");
    buttons_arr.className="flex gap-2 justify-end absolute right-2 -top-[5px] bg-slate-100 rounded-bl-xl rounded-tr-xl py-1 px-2 mt-[4px] mr-[-8px]";
    ...
    edit_btn.className="rounded-full h-4 w-4 bg-white border-blue-400 border-solid border-[1px] flex justify-center items-center ";
    buttons_arr.appendChild(edit_btn);
    let edit_span=document.createElement("span");
    edit_span.className="fa fa-edit text-blue-400 text-[9px] ml-[1px] mt-[1px]";
    ...
    delete_btn.className="rounded-full h-4 w-4 border-red-500 bg-white border-solid border-[1px] flex justify-center items-center";
    buttons_arr.appendChild(delete_btn);
    let delete_span=document.createElement("span");
    delete_span.className="fa fa-trash text-red-500 text-[9px]";
    delete_btn.appendChild(delete_span);

    // name
    let fullname=document.createElement("h3");
    fullname.className="font-nunito xs:text-sm md:text-xs";
    details_div.appendChild(fullname);
    let fullname_text=document.createTextNode(nametext);
    fullname.appendChild(fullname_text);

    // phone
    let phone=document.createElement("h3");
    phone.className="font-nunito xs:text-[10px] text-xs md:text-[10px] text-gray-600";
    details_div.appendChild(phone);
    let phone_text=document.createTextNode(`+91 ${phonetext}`);
    phone.appendChild(phone_text);

    // email
    let email=document.createElement("h3");
    email.className="font-nunito xs:text-[8.5px] text-xs text-gray-600";
    details_div.appendChild(email);
...
function validateAddForm(){
    ...
    function checkName(){
        ...
        }else if(name1.value.trim().length>15){
            name1.style.outlineColor="red";
            name_error.style.display="block";
            name_error.textContent="Maximum 15 characters allowed !";
        }else{
            checkEmail();
